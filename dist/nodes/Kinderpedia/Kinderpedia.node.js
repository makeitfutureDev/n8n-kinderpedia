        ...
        this.methods = {
            loadOptions: {
                ...
                async getCustomFieldsForEntity() {
                    ...
                    const entityType = this.getCurrentNodeParameter('updateEntityType');
                    if (!entityType) {
                        return [
                            {
            ...
            try {
                ...
                if (resource === 'student') {
                    ...
                    if (operation === 'getAll') {
                        ...
                        const limit = !returnAll ? this.getNodeParameter('limit', i) : null;
                        let page = 1;
                        let hasMore = true;
                        while (hasMore) {
                            const response = await this.helpers.request({
                ...
                if (resource === 'parent') {
                    ...
                    // Create Parent
                    if (operation === 'create') {
                        const emailInfo = this.getNodeParameter('emailInfo', i);
                        const firstName = this.getNodeParameter('firstName', i);
                        const lastName = this.getNodeParameter('lastName', i);
                        const phone = this.getNodeParameter('phone', i, '');
                        const address = this.getNodeParameter('address', i, '');
                        const familyId = this.getNodeParameter('familyId', i);
                        const familyAcctype = this.getNodeParameter('familyAcctype', i);
                        const familyMainAccount = this.getNodeParameter('familyMainAccount', i);
                        const familyPickup = this.getNodeParameter('familyPickup', i);
                        const body = {
                            emailInfo,
                            firstName,
                            lastName,
                ...
                if (resource === 'customField') {
                    ...
                    if (operation === 'getValues') {
                        const entityType = this.getNodeParameter('entityType', i);
                        const entityId = this.getNodeParameter('entityId', i, 0);
                    ...
                    if (operation === 'addValue') {
                        ...
                        if (!attributeId || attributeId === 0) {
                            throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'Attribute ID is required and must be a valid ID', { itemIndex: i });
                        }
                        if (!entityId || entityId === 0) {
                            throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'Entity ID is required and must be a valid ID', { itemIndex: i });
                        }
                        if (!value || value.trim() === '') {
                            throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'Value is required and cannot be empty', { itemIndex: i });
                        }
                        const body = {
                            attributeId,
                            entityId,
                    ...
                    if (operation === 'updateValue') {
                        ...
                        if (fields.length === 0) {
                            throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'At least one custom field must be specified', { itemIndex: i });
                        }
                        ...
                        for (const field of fields) {
                            ...
                            // Validate field data
                            if (!attributeId || attributeId === 0) {
                                throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'Attribute ID is required for each custom field', { itemIndex: i });
                            }
                            if (!value || value.trim() === '') {
                                throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'Value is required for each custom field', { itemIndex: i });
                            }
                            const body = {
                                attributeId,
                                entityId,
                    ...
                    if (operation === 'deleteValue') {
                        ...
                        try {
                            ...
                            if (error.statusCode === 404 || ((_a = error.response) === null || _a === void 0 ? void 0 : _a.status) === 404) {
...
exports.Kinderpedia = Kinderpedia;
